// The Seven Realms — progression engine. Realm/stage are NEVER stored; always computed here
// from `hammerCount`. This MUST stay in lockstep with the app's `src/constants/realms.ts`.
// (Single behavioural contract; both sides are unit-tested against the same thresholds.)
...
export interface Realm {
  ...
  sigil: string;
  stageLabel: string; // entity evolution stage name
}
...
export interface RealmState {
  realm: Realm;
  stage: number; // == realm.index (1..7); the entity's evolution stage
  hammerCount: number;
  originArtMastery: number; // hammerCount * 0.001
  nextThreshold: number | null; // null at Divine Master
  hammerToNext: number | null;
  progressToNext: number; // 0..1 within the current realm band
}

/** Pure: the realm/stage for a given lifetime hammerCount. Never stored — always derived. */
export function realmForHammerCount(hammerCount: number): RealmState {
  ...
  const next = REALMS[realm.index]; // realm.index is 1-based, so this is the next realm
...
/** Did a hammerCount delta cross one or more realm thresholds? Returns crossed realms (for ascension). */
