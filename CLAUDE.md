...
18. **Ascension is standard-gated and community-verified — never bought, spoofed, or stored** (`GAME_DESIGN.md §16`). `depthRealm` derives from `hammerCount` (invariant #1 unchanged); `provenRealm` derives from append-only `ProvingEvent`s (a per-Art **Standard** met, attested); `realm` = a pure function of both (`lib/standards.ts`, generalizes `realmForHammerCount`) — still never stored. `depthRealm > provenRealm` ⇒ **at the Threshold/Bottleneck** (a celebrated state, never a demotion; grandfathered depth = `claimed` standing — zero migration loss). Manual effort builds **depth/foundation**; a **Proving** requires **attestation** (`claimed` → `corroborated` via health-plausibility → `witnessed` via a Sect peer → `certified` via a real event). Verification is an anti-spoof + amplifier, **never a hard sensor-gate**: a consistent, plausible self-attested Proving is the floor (offline/sensor-optional preserved, L5). Standards/Provings/Sect-entry are PROGRESSION ⇒ free, unbuyable, additive. **Belonging, not ranking:** Cohorts (auto, derived — peers who proved the same realm) and Sects (opt-in, `entryStandard`-gated, mutual witnesses) are never competitive ladders or PvP. `StrikeEvent.source`/attestation weights Standing, never depth — `hammerCount` is still Σ `amount`.
...
## Ascension Standards & the Cohort (S1–S2, design-specced in GAME_DESIGN §16)
The fix for "grind manual reps to ascend": realm gains two axes. **depthRealm** = `realmForHammerCount(hammerCount)` (unchanged — foundation, never lost). **provenRealm** = highest realm with an attested **Proving** (append-only `ProvingEvent`); `realm` derives from both via `lib/standards.ts realmFor()` (pure, generalizes `realmForHammerCount`, never stored). `depthRealm > provenRealm` ⇒ **at the Threshold** (qi amassed, breakthrough open). A **Standard** is a per-Art-family normalized score (DOTS-style for strength, age-graded pace for cardio, etc. — `constants/standards.ts`, percentile-tier bands → the seven realms). A **Gate IS a Proving**; the Breakthrough Gate is the realm Proving. **Attestation:** `claimed` (self, plausible) → `corroborated` (health-data plausibility, the anti-spoof) → `witnessed` (Sect peer co-sign) → `certified` (real event); a pure **plausibility engine** holds impossible efforts out of Provings (depth still counts). **Cohort** = derived peers who proved a realm (belonging, never ranking); **Sect** gains `entryStandard` (the "meet the requirement to join" — opt-in, witness-based, never sold). Free/unbuyable/sensor-optional; grandfathered depth = `claimed`. `StrikeEvent.source` weights Standing, never depth.
...
- **Standards (S):** `realmFor()` is pure and recomputes identically from `hammerCount` + `ProvingEvent`s (a v1 account = `claimed` standing at its depth realm, ZERO demotion); a Proving requires attestation ≥ `claimed` and passes the plausibility engine; manual junk volume raises depth but cannot cross a Threshold; Standards/Provings/Sect-entry carry no priceModel and never resolve via gacha; the Cohort exposes no competitive ranking; a phone-only account can still prove (self-attested floor).
